...
class GmmMeshStage:
    ...
    def get_gmm_doriro(self, gaussians) -> Tuple[TS, T]:
        # gaussians = gaussians[:-1]
        raw_gmm = [g.get_raw_data() for g in gaussians]
        phi = torch.tensor([g[0] for g in raw_gmm], dtype=torch.float32).view(1, 1, -1)
        # phi = torch.from_numpy(self.raw_gmm[0]).view(1, 1, -1).float()
        mu = torch.stack([torch.from_numpy(g[1]).float() for g in raw_gmm], dim=0).view(1, 1, -1, 3)
        p = torch.stack([torch.from_numpy(g[3]).float() for g in raw_gmm], dim=0).view(1, 1, -1, 3, 3)
        eigen = torch.stack([torch.from_numpy(g[2]).float() for g in raw_gmm], dim=0).view(1, 1, -1, 3)
        gmm = mu, p, phi, eigen
        included = torch.tensor([g.gaussian_id for g in gaussians], dtype=torch.int64)
        return gmm, included
    ...
    # TODO DORI loads the given mesh shape_id as obj and gmm. maybe here we could save.
...
class MeshGmmStatuses(GmmStatuses):
    ...
    def save_light_doriro(self, root, gmms, gaussians, shape_id):

        gmms = self.sort_gmms(*gmms)
        save_dict = {'ids': {
                             shape_id: [g.gaussian_id[1] for g in gaussians]},
                     'gmm': gmms}
        path = f"{root}/{files_utils.get_time_name(shape_id)}"
        files_utils.save_pickle(save_dict, path)
        print("saved!")
