...
def export_gmm(gmm: TS, item: int, file_name: str, included: Optional[List[int]] = None, time=True):
    if included is None:
        included = [1] * gmm[0].shape[2]
    mu, p, phi, eigen = [tensor[item, 0].flatten().cpu() for tensor in gmm]
    # phi = phi.softmax(0)
    if time:
        file_name = get_time_name(file_name)
...
def export_mesh(mesh: Union[V_Mesh, T_Mesh, T, Tuple[T, List[List[int]]]], file_name: str,
                colors: Optional[COLORS] = None, normals: TN = None, edges=None, spheres=None, time=True):
    # return
    print("exporting mesh")
    if time:
        file_name = get_time_name_obj(file_name)
...
@path_init('.obj', 1, True)
def export_mesh_edelman(mesh: Union[V_Mesh, T_Mesh, T, Tuple[T, List[List[int]]]], file_name: str,
                colors: Optional[COLORS] = None, normals: TN = None, edges=None, spheres=None, time=True):
    # return
    if time:
        file_name = get_time_name_obj(file_name)
...
def get_time_name_obj(name: str, format_="%m_%d-%H_%M") -> str:
    # split str to 2.
    a, b = name.split('.')
    return f'{a}_{time.strftime(format_)}_.{b}'
