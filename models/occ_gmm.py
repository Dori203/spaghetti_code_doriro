...
class OccGen(models_utils.Model):
    ...
    #     z = self.get_z(items)
    #     z_between = self.interpolate_(z, num_between)
    #     zh, gmms, none = self.occ_former(z_between)
    #     # result = self.occ_former(z_between)
    #     print("fish")
    #     return zh, gmms
    ...
    def get_embeddings(self, item: T, noise: bool=False):
        z = self.get_z(item)
        # z = z * 3
        # print(z.min(), z.median(), z.max())
        # TODO DORIRO add noise here.
        if(noise):
            z = z * 2
    ...
    def get_random_embeddings(self, num_items: int, noise: float=0, only_noise = False):
        if self.dist is None:
            weights = self.z.weight.clone().detach()
            mean = weights.mean(0)
            # mean_2 = torch.ones(mean.shape)
            # mean = mean - mean_2
            # mean = mean * 20
            # print("updated mean")
        ...
        random_nums = torch.rand(z_init.shape, device=self.device)
        z_init += noise * random_nums
        if(only_noise):
            z_init = random_nums * noise
            z_init -= z_init.mean()
            z_init += mean
        return z_init
