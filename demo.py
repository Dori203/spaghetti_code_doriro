...
class RenderMain(StagedCanvas):
    ...
    def update_gmm_doriro(self, stage: ui_controllers.GmmMeshStage, changed: List[int]):
        for item in changed:
            is_toggled, toggled = stage.toggle_inclusion_by_id(item, True)
            is_toggled, toggled = stage.toggle_inclusion_by_id_doriro(item, True)
            new_addresses = self.stage.add_gaussians(toggled)
            for gaussian, new_address in zip(toggled, new_addresses):
                self.stage_mapper[gaussian.get_address()] = new_address
            self.make_twins(toggled, new_addresses)
...
class MeshScroller(vtk.vtkRenderer):
    ...
    def filter_gmms(self, shape_index, list_of_indices):
        gmms = self.meshes.gmms[shape_index].gmm
        print(len(gmms))
        filtered_gmms = [gmms[i] for i in list_of_indices]
        return filtered_gmms
...
class InteractorStyle(vtk.vtkInteractorStyleTrackballCamera):
    ...
    def on_key_press(self, obj, event):
        ...
        if key == 'return' or key == 'kp_enter':
            ...
            # START THE CHANGE
            # gaussian_list = self.mesh_scroller.meshes.gmms[0] #gaussianStatus
            # gmms_2 = self.render_main.stage.get_gmm_doriro(gaussian_list.gmm)
            # root = self.root
            # self.mesh_scroller.meshes.save_light_doriro(self.root, gmms_2, gaussian_list.gmm, gaussian_list.shape_id)

            # self.render_main.save(self.root)
        # if key == 'q':
        #     for i in range(len(self.mesh_scroller.meshes.gmms)): #TODO DORIRO
        #         shape_id = self.mesh_scroller.meshes.gmms[i].shape_id #gaussianStatus
        #         num_of_indices = random.randint(8,14)
        #         indices = np.random.choice(16, num_of_indices)
        #         indices.sort()
        #         # print(indices)
        #         gaussian_gmms = self.mesh_scroller.filter_gmms(i, indices)
        #         gmms_2 = self.render_main.stage.get_gmm_doriro(gaussian_gmms)
        #         root = self.root
        #         self.mesh_scroller.meshes.save_light_doriro(self.root, gmms_2, gaussian_gmms, shape_id)
        if key == 'w':
            for i in range(len(self.mesh_scroller.meshes.gmms)):
                shape_id = self.mesh_scroller.meshes.gmms[i].shape_id #gaussianStatus
                ...
                self.mesh_scroller.meshes.save_light_doriro(self.root, gmms_2, gaussian_gmms, shape_id)
        if key == 'a':
                print('trying to save something random at root')
                print(self.root)
                # self.mesh_scroller.meshes.save(self.root,gmms=[self.mesh_scroller.meshes.gmms])
                # self.mesh_scroller.save(self.root, gmms=[self.mesh_scroller.meshes.gmms])
                # self.mesh_scroller.save(self.root)
                # ui_controllers.GmmMeshStage.save()
                self.render_main.stage.save(self.root)
                # self.selected_view.save(self.root)
                # pass
        if key == 'control_l':
...
def main(with_model: bool = True):
    ...
    prefix = ''
    if 0:
        shape_ids = (188, 4814, 2954, 3631, 3327, 4410, 5551, 1103, 1637,
                     2117, 2959, 3718, 4958, 5109, 5710, 6063, 6129, 6240, 6567)
    else:
